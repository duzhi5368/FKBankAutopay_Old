...
namespace FKVerifyLib
{
    public class CodeHelper
    {
        ...
        /// 加载验证码配置信息
        /// </summary>
        /// <param name="strFileName">验证码配置文件</param>
        ...
        /// </summary>
        /// <param name="imgSrc"></param>
        /// <param name="nNoiseLv">噪音强度 4强力 3普通 1仅去噪点 0不处理</param>
        /// <returns></returns>
        public Image ClearNoise(Image imgSrc, int nNoiseLv)
        {
            return ClearNoiseStep(imgSrc, 128, nNoiseLv);
        }
        /// <summary>
        /// 单次去除噪音
        /// </summary>
        /// <param name="imgSrc"></param>
        /// <param name="nGrayValue"></param>
        /// <param name="nMaxNearPoints"></param>
        /// <returns></returns>
        private Image ClearNoiseStep(Image imgSrc, int nGrayValue, int nMaxNearPoints)
        {
            Bitmap b = new Bitmap(imgSrc);
            Bitmap bmp = b.Clone(new Rectangle(0, 0, imgSrc.Width, imgSrc.Height), PixelFormat.Format24bppRgb);
            b.Dispose();
            ...
            for (int i = 0; i < bmp.Width; ++i)
            {
                ...
                {
                    bR = byColorInfo[(j) * bmpData.Stride + (i) * 3 + 2];
                    if (bR < nGrayValue)
                    {
                        nNearDots = 0;
                        ...
                        {
                            byColorInfo[(j) * bmpData.Stride + (i) * 3 + 0] =
                            byColorInfo[(j) * bmpData.Stride + (i) * 3 + 1] =
                            byColorInfo[(j) * bmpData.Stride + (i) * 3 + 2] = 255;
                        }
                        else
                        {
                            if (byColorInfo[(j - 1) * bmpData.Stride + (i - 1) * 3 + 2] < nGrayValue)
                                nNearDots++;
                            if (byColorInfo[(j - 1) * bmpData.Stride + (i) * 3 + 2] < nGrayValue)
                                nNearDots++;
                            if (byColorInfo[(j - 1) * bmpData.Stride + (i + 1) * 3 + 2] < nGrayValue)
                                nNearDots++;
                            if (byColorInfo[(j) * bmpData.Stride + (i - 1) * 3 + 2] < nGrayValue)
                                nNearDots++;
                            if (byColorInfo[(j) * bmpData.Stride + (i + 1) * 3 + 2] < nGrayValue)
                                nNearDots++;
                            if (byColorInfo[(j + 1) * bmpData.Stride + (i - 1) * 3 + 2] < nGrayValue)
        ...
        /// </summary>
        /// <param name="imgSrc"></param>
        /// <returns>去噪后的图</returns>
        public Image ClearNoiseByMidValue(Image imgSrc)
        {
            byte[] p = new byte[9];     // 3 * 3
            ...
            for (int y = 1; y < bmp.Height - 1; y++)
            {
                ...
                {
                    // 取9个点的R值
                    p[0] = byColorInfo[(y-1) * bmpData.Stride + (x-1) * 3 + 2];     //bmp.GetPixel(x - 1, y - 1).R;
                    p[1] = byColorInfo[(y-1) * bmpData.Stride + (x) * 3 + 2];       //bmp.GetPixel(x, y - 1).R;
                    p[2] = byColorInfo[(y-1) * bmpData.Stride + (x+1) * 3 + 2];     //bmp.GetPixel(x + 1, y - 1).R;
                    p[0] = byColorInfo[(y) * bmpData.Stride + (x-1) * 3 + 2];       //bmp.GetPixel(x - 1, y).R;
                    p[0] = byColorInfo[(y) * bmpData.Stride + (x) * 3 + 2];         //bmp.GetPixel(x, y).R;
                    p[0] = byColorInfo[(y) * bmpData.Stride + (x+1) * 3 + 2];       //bmp.GetPixel(x + 1, y).R;
                    p[0] = byColorInfo[(y+1) * bmpData.Stride + (x-1) * 3 + 2];     //bmp.GetPixel(x - 1, y + 1).R;
                    p[0] = byColorInfo[(y+1) * bmpData.Stride + (x) * 3 + 2];       //bmp.GetPixel(x, y + 1).R;
                    p[0] = byColorInfo[(y + 1) * bmpData.Stride + (x + 1) * 3 + 2]; //bmp.GetPixel(x + 1, y + 1).R;
                    // 计算中值
                    for (int j = 0; j < 5; ++j)
                    {
                    ...
                    // 给有效值赋中值
                    byColorInfo[(y) * bmpData.Stride + (x) * 3 + 0] = 
                    byColorInfo[(y) * bmpData.Stride + (x) * 3 + 1] =
                    byColorInfo[(y) * bmpData.Stride + (x) * 3 + 2] = p[4];
                    //bmp.SetPixel(x, y, Color.FromArgb(p[4], p[4], p[4]));
                }
            }
            Marshal.Copy(byColorInfo, 0, bmpData.Scan0, byColorInfo.Length);
            bmp.UnlockBits(bmpData);
