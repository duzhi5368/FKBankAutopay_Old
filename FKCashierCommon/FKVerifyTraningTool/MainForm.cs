...
namespace FKVerifyTraningTool
{
    ...
    {
        #region 类变量
        private CodeHelper  GlobalCH = new CodeHelper();
        private Image SrcImage = null;
        private Image BinaryImage = null;
        private Image ReverseImage = null;
        ...
        private void groupBox2_Paint(object sender, PaintEventArgs e)
        {
            ...
            if(ReverseImage != null)
            {
                ...
                e.Graphics.DrawString("反色图像", textFont, textBrush, 370 - 4 * 20, y + 10 + SrcImage.Height * 2 + 5 + BinaryImage.Height * 2);
            }
            if(CharRects.Count > 0)
            {
                Rectangle srcRect = new Rectangle(x + 5, y + 10 + SrcImage.Height * 2 + 5 + BinaryImage.Height * 2 + 5 + ReverseImage.Height * 2,
                    ReverseImage.Width * 2, ReverseImage.Height * 2);
                e.Graphics.DrawImage(ReverseImage, srcRect);
                e.Graphics.DrawString("区域选择", textFont, textBrush, 370 - 4 * 20, y + 10 + SrcImage.Height * 2 + 5 + BinaryImage.Height * 2 + 5 + ReverseImage.Height * 2);
                ...
                for (int i = 0; i < CharRects.Count; ++i)
                {
                    ...
                    rect.Height *= 2;
                    rect.X += x + 5;
                    rect.Y += y + 10 + SrcImage.Height * 2 + 5 + BinaryImage.Height * 2 + 5 + ReverseImage.Height * 2;
            ...
            if(CharImages.Count > 0)
            {
                e.Graphics.DrawString("最终字符", textFont, textBrush, 370 - 4 * 20, 
                    y + 10 + SrcImage.Height * 2 + 5 + BinaryImage.Height * 2 + 5 + BinaryImage.Height * 2 + 5 + ReverseImage.Height * 2 );
                int nXBegin = x + 5;
                for (int i = 0; i < CharImages.Count; ++i)
                {
                    Rectangle srcRect = new Rectangle(nXBegin, y + 10 + SrcImage.Height * 2 + 5 + BinaryImage.Height * 2 + 5 + ReverseImage.Height * 2 + 5 + ReverseImage.Height * 2,
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                LoadVerifyPic(resultFile, false);
            }
        }
        ...
        private void button5_Click_1(object sender, EventArgs e)
        {
            ...
            {
                LoadVerifyPic(string.Format(strFormat, i), false);

                TrainBoxForm form = new TrainBoxForm();
                form.ShowDialog();
        ...
        public void LoadConfigFromFile(string strFile)
        {
            ...
            //int nValue = GlobalCH.m_ci.BinaryValues[0];
            //textBox2.Text = trackBar1.Value.ToString();
        ...
        public void LoadVerifyPic(string strPath, bool bUseReverseColor)
        {
            ...
            if (bUseReverseColor)
                ReverseImage = GlobalCH.ReverseColors(tmpBinaryImage);
            else
                ReverseImage = tmpBinaryImage;
            CharRects.Clear();
            CharImages.Clear();
            CharRects = GlobalCH.GetCharRect(ReverseImage, GlobalCH.m_ci.PixelMin, GlobalCH.m_ci.PixelMax);
